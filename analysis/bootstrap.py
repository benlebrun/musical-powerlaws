...
def bootstrap(dist, semi_parametric=True, niter=1e3, set_xmin=False, quiet=False, dest=False, notebook=False):
	...
	for B in tqdm(range(0, niter)):
		...
		# fit synthetic data
		if isinstance(set_xmin, bool):
			syn_dist = dist.get_obj()
			syn_fit = syn_dist(synthetic_data)
		else:
			syn_dist = dist.get_obj(xmin=True)
			syn_fit = syn_dist([synthetic_data, set_xmin])
		stats = [np.mean(synthetic_data), np.std(synthetic_data), np.median(synthetic_data), syn_fit.D, syn_fit.xmin]
				 #kl_divergence(syn_fit.epdf(), syn_fit.pdf()),  kl_divergence(syn_fit.pdf(), syn_fit.epdf()), syn_fit.xmin]
...
def main(btype, source, dest, niter=1e3):
	...
	fit = Powerlaw_with_cutoff(ranks)
	print('done.')
	...
	cols = ['mean','std','median', 'D','xmin', 'KL1', 'KL2']
